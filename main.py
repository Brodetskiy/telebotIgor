...
def start (message):
    if message.text =='/reg':
        ...
        cursor.execute(
            'create table other.registration(  id int GENERATED BY DEFAULT AS IDENTITY primary key, name varchar (150),surname varchar(150),father_name varchar(150),number int,email varchar(200),  bank varchar(300),  position varchar(100),  dateCreate date default now() )')

        cursor.execute(
            'create procedure other.useRegistration (name varchar (150), surname varchar(150), father_name varchar(150),  number int,email varchar(200), bank varchar(300), positions varchar(100))language sql as  $$ insert into other.registration (name, surname, father_name, number, email, bank, position) select name, surname, father_name, number, email, bank, positions $$')
